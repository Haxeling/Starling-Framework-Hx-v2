...
class MathUtil
{
    ...
    {
        if (Std.is(Float, Int) && Float > 0 && (Float & (Float - 1)) == 0)               // see: http://goo.gl/D9kPj  
        return Float
        else 
        {
            var result : Int = 1;
            Float -= 0.000000001;  // avoid floating point rounding errors  
            
            while (result < Float)result <<= 1;
            return result;
        }
    }
    ...
    {
        return value < (min != 0) ? min : (value > (max != 0) ? max : value);
    }
}
