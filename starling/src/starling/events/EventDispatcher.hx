...
class EventDispatcher
{
    ...
    public function addEventListener(type : String, listener : Function) : Void
    {
        ...
        var listeners : Array<Function> = try cast(Reflect.field(_eventListeners, type), Array/*Vector.<T> call?*/) catch(e:Dynamic) null;
        if (listeners == null) 
    ...
    public function removeEventListener(type : String, listener : Function) : Void
    {
        if (_eventListeners != null) 
        {
            ...
            if (numListeners > 0) 
            {
                ...
                if (index != -1) 
                {
                    var restListeners : Array<Function> = listeners.substring(0, index);
    ...
    public function dispatchEvent(event : Event) : Void
    {
        ...
        if (!bubbles && (_eventListeners == null || !(Lambda.has(_eventListeners, event.type)))) 
            return  // this allows users to re-dispatch events without creating a clone.    // we save the current target and restore it later;    // no need to do anything  ;
    ...
    private function invokeEvent(event : Event) : Bool
    {
        ...
        try cast(_eventListeners[event.type], Array/*Vector.<T> call?*/) catch(e:Dynamic) null : null;
        ...
        if (numListeners != 0) 
        {
            ...
            for (i in 0...numListeners){
                var listener : Function = try cast(listeners[i], Function) catch(e:Dynamic) null;
    ...
    private function bubbleEvent(event : Event) : Void
    {
        ...
        chain[as3hx.Compat.parseInt(length++)] = element;
